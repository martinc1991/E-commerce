// <------------------------------------ IMPORTS ------------------------------------>
// React
import { faMinus, faPlus, faStar, faThumbsDown, faThumbsUp } from '@fortawesome/free-solid-svg-icons';
// Iconos
...
export default function Reviews({ product, getProductP, userReviews, getUserReviews, deleteReviewP, userLoggedP, editReview, setEditReview, handlerRate, editReviewForm, handlerEditReview, rating }) {
			...
			<>
				<Card className={` p-4 ${s.productReviewCard}`}>
					...
					<Row className={`${s.productReviewHeader} justify-content-center justify-content-md-start w-100 m-0`}>
						...
						<Col xs={12} md={4}>
							<h6 onClick={openMyReviews}>Gestionar mis comentarios</h6>
						</Col>
					</Row>
				...
				<MyReviews show={show} setShow={setShow} product={product} user={userLoggedP} userReviews={userReviews} getUserReviews={getUserReviews} deleteReviewP={deleteReviewP} closeMyReviews={closeMyReviews} editReview={editReview} setEditReview={setEditReview} handlerRate={handlerRate} editReviewForm={editReviewForm} handlerEditReview={handlerEditReview} />
			</>
		);
	}
}
