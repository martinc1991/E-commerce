		...
		<Card className={stock === 0 ? ` m-2 flex-dark  ${s.productRunOut}` : ` m-2 flex-dark ${s.productCard}`}>
			...
			<Card.Body className={s.cont_prin}>
				...
				<div className={s.contPI}>
					<Card.Text className={`my-1 ${s.productCardPrice}`}>{stock === 0 ? '' : `${price}` || `Product Name`}</Card.Text>
					<div className={s.cont_I}>
						{stock === 0 ? (
							...
							<>
								<Button
									onClick={() => {
										addToCartP(id, 1, userLoggedP ? userLoggedP.id : 1);
										setShow(true);
									}}
									className={`mb-1 ${s.productCardButton2}`}
								>
									<div>
			...
			<div className={s.fixedRate}>
				<h6>
					{fixedRate} ({reviews && reviews.length} opiniones)
				</h6>
			</div>
		</Card>
	);
