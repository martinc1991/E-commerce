...
const MyReviews = ({ show, setShow, product, user, userReviews, getUserReviews, deleteReviewP, closeMyReviews, editReview, setEditReview, handlerRate, editReviewForm, handlerEditReview }) => {
	useEffect(() => {
		if (user) {
	...
	return (
		<>
			<Modal show={show} onHide={() => setShow(false)} dialogClassName={s.cont_print} centered aria-labelledby='example-custom-modal-styling-title'>
					...
					<Row className={s.cont}>
						...
						<Col xs={6} md={8} className={s.cont_inf}>
							<div className={s.info}>
								...
								<Modal.Footer>
									<Button variant='success' onClick={() => closeMyReviews()}>
										done!
									</Button>
								</Modal.Footer>
							</div>
			...
			<EditReview user={user} show={showEdit} setShow={setShowEdit} product={product} review={editReview} getUserReviews={getUserReviews} handlerRate={handlerRate} editReviewForm={editReviewForm} handlerEditReview={handlerEditReview} />
		</>
	);
};
